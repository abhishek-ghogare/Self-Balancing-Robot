...
void Timer0IntHandler(void)
{
	...
	int32_t pin0 = GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_0 | GPIO_PIN_4) & GPIO_PIN_0;
	int32_t pin4 = GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_4 | GPIO_PIN_4) & GPIO_PIN_4;
	...
	if (btn_sw1_state == 1) {				// If sw1 button is in state 1, i.e. if sw1 was pressed 10ms earlier
		if (!pin4) {						// If pin4 is low, i.e. if sw1 is pressed
	...
	if (btn_sw1_state == 2 && pin4) {		// If button was already in state 2 and pin4 is high now i.e. button is not pressed now
	    (*btn_onSW1ButtonUpHdr)();			// Execute button up handler
		btn_onSW1ButtonUp();				// Reset button state
	}

	if (btn_sw2_state == 1) {				// If sw2 button is in state 1, i.e. if sw2 was pressed 10ms earlier
		if (!pin0) {						// If pin0 is low, i.e. if sw2 is pressed
	...
	if (btn_sw2_state == 2 && pin0) {		// If button was already in state 2 and pin0 is high now i.e. button is not pressed now
